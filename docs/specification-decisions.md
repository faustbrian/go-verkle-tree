...
[research profile v0](../specification/bandersnatch-ipa-256-v0.md), the
[profile-freeze decision](../specification/profile-freeze.md), and the bounded
[compatibility matrix](compatibility.md).

## VERKLE-DEC-001: Research profile-v0 identity and interoperability boundary
...
| Credible interpretations | Freeze a stable Ethereum profile, expose independently configurable cryptographic components, or retain one immutable package-owned research profile v0 whose external claims are limited to pinned corpora. |
...
| Selected behavior | `verkletree-bandersnatch-ipa-256-v0` is the only implemented profile identity. Its width, key layout, curve and group, generator set, transcript, encodings, and state semantics are not caller-composable. The Go API is stable v1 around this research profile; it makes no production-suitability, external-audit, or Ethereum-protocol compatibility claim. |
...
This register does not close a production-profile freeze, production
suitability, external audit, general reference-client compatibility,
or Ethereum protocol compatibility; those remain production-adoption blockers
or explicit non-claims until separately decided and evidenced.
